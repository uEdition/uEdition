...
### options
- `build-mode`: If set to `full`, a fresh environment will be used for the build process. `partial` only rebuilds
  changed entries. Defaults to `True`.
...
the build command with the option `--build-mode=full`.
